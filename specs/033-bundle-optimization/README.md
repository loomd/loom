...
## Requirements
- [x] 将 `TerminalTab` 改为动态导入：
  - `const TerminalTab = React.lazy(() => import('./components/TerminalTab'))`
  - 包裹 `<Suspense fallback={<TerminalPlaceholder />}>`
- [x] 将 `FileEditor` 改为动态导入：
  - `const FileEditor = React.lazy(() => import('./components/FileEditor'))`
  - 包裹 `<Suspense fallback={<EditorPlaceholder />}>`
- [x] 在 `ProjectWorkspace.tsx` 中应用 lazy 加载到终端和编辑器
- [x] 提取 `TerminalPlaceholder` 组件（简约 loading 骨架屏，高度匹配终端区域）
- [x] 提取 `EditorPlaceholder` 组件（简约 loading 骨架屏，高度匹配编辑器区域）
- [x] 验证 Monaco Editor 的 `@monaco-editor/react` 是否按预期 code-split（需检查其内部加载行为，避免双重加载）
- [x] 可选评估：将 `RightSidebar` 中的 `GlobalSkillModal` / `GlobalDocModal` 也做懒加载（操作路径更深）
...
## Acceptance Criteria
- [x] 应用初始加载时不请求 xterm.js 和 Monaco Editor 的 chunk
- [x] 打开终端 tab 时才加载 xterm.js chunk（可见 loading 占位）
- [x] 打开文件编辑器时才加载 Monaco Editor chunk（可见 loading 占位）
- [x] 终端/编辑器功能正常（交互、样式、xterm addon）
- [x] `npm run build` 产物应产生多个 JS chunk（而非单个 bundle）
