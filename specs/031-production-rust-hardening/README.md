...
## Requirements
- [x] 定位 `manager.rs` 中所有非测试代码的 `.unwrap()` 调用（约 10 处）
- [x] 逐个分析每个 unwrap 的上下文，判断应使用：
...
- [x] 重点关注的调用点（基于扫描结果）：
  ...
  - 第 2180 行附近
  - 第 2296 行附近
- [x] 替换后确保错误信息包含足够的上下文（哪个操作、哪个资源）
- [x] 运行 `cargo test` 确认现有测试不受影响
...
## Acceptance Criteria
- [x] 所有生产代码路径上的 `.unwrap()` 被消除
- [x] `cargo clippy --all-targets` 无新增警告（允许既有的 clippy 规则）
- [x] `cargo test` 全部通过
- [x] 错误发生时进程不再 panic，而是返回 `Err` 给调用方
