...
## Requirements
- [x] 清理 `TerminalTab.tsx` 中所有 `console.log` / `console.warn` 调试语句（约 20+ 处，集中在 IME 位置计算逻辑）
...
## Acceptance Criteria
- [x] 终端不再打印调试日志（正常使用场景）
- [x] 切换设置 tab 时不可见的 tab 组件从 DOM 中卸载
- [x] `main.rs` 导入从 40+ 行减少到 <5 行
- [x] `npm run build` 无错误
- [x] `npm run lint` 无错误
- [x] `cargo build` 无错误
- [x] 终端 IME 输入中日韩文字正常工作（回归验证）
