...
## Requirements
- [x] 确认 `loom_core` 中 `expand_env_vars` 的可见性（当前是否为 `pub`）
- [x] 若非 `pub`，将其改为 `pub` 并确保 `lib.rs` 或其 `storage/mod.rs` 重新导出
- [x] 删除 `crates/gui/src-tauri/src/pty.rs` 中的重复实现
- [x] 在 `pty.rs` 中改用 `use loom_core::storage::expand_env_vars;`（或 `loom_core::expand_env_vars`，取决于导出路径）
- [x] 验证两端功能一致（检查两处实现是否有细微差异）
  - 若存在差异，统一为更完善的版本
- [x] 搜索是否还有其他跨 crate 重复代码
...
## Acceptance Criteria
- [x] `pty.rs` 中不再包含 `expand_env_vars` 的独立实现
- [x] GUI 终端的环境变量展开行为与 CLI 一致（回归测试）
- [x] `cargo build` 无错误
- [x] `cargo test` 全部通过
