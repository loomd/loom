...
## Requirements
- [x] 为 `utils.ts` 工具函数写测试（重点是 `mergeCliArgs` 的复杂逻辑）：
  - 数组模式、对象模式
  - 空输入
  - 参数覆盖优先级
- [x] 为 `api.ts` 包装器写测试：
  ...
  - mock `invoke` 返回值
  - 错误传播
- [x] 为 `TerminalTab.tsx` 写测试：
  - 渲染终端容器
  - 标签页切换
  - 关闭终端
  - mock xterm.js 的 `@xterm/xterm` 模块
- [x] 为 `FileEditor.tsx` 写测试：
  - 渲染编辑器
  - 语言选择
  - 保存操作
  - mock `@monaco-editor/react`
- [x] 为 `RightSidebar.tsx` 写测试：
  - 技能/文档 Tab 渲染
  - 点击打开 modal
- [x] 为 `App.tsx` 写测试（分解后可测性提升后补充）：
  - 项目列表渲染
  - 页面切换
  - 主题切换
- [x] 检查并修复所有跳过的 e2e 测试（`gui.test.ts` 中 6 个 `.skip`）：
...
## Acceptance Criteria
- [x] 核心工具函数 `mergeCliArgs` 有完整测试
- [x] `TerminalTab` 有基本渲染和交互测试
- [x] `FileEditor` 有基本渲染测试
- [x] `RightSidebar` 有基本渲染测试
- [x] `api.ts` 的每个导出函数有 mock 调用测试
- [x] `vitest run` 全部通过
- [x] 测试运行时间无明显增加（控制在 <30s）
