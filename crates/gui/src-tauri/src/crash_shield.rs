...
fn log_webview_fail(kind: &str) {
    ...
    let entry = format!(
        "[{time_str}] [WEBVIEW_FAIL] ts={unix_ts} uptime={uptime} version={version} kind={kind} restart_count={count} pid={pid}\n",
        pid = std::process::id()
    );
...
/// 监听 WebView2 渲染进程异常退出或卡死（如系统低内存 OOM 被杀），自动执行原地自愈重载
#[cfg(target_os = "windows")]
pub fn install_webview_shield(window: &tauri::WebviewWindow) {
    use std::sync::atomic::{AtomicU64, Ordering};
    use webview2_com::Microsoft::Web::WebView2::Win32::*;
    use webview2_com::ProcessFailedEventHandler;

    static LAST_HEAL_MS: AtomicU64 = AtomicU64::new(0);
    const MIN_HEAL_INTERVAL_MS: u64 = 5000;
        ...
        unsafe {
            ...
            let handler = ProcessFailedEventHandler::create(Box::new(
                move |_sender, args: Option<ICoreWebView2ProcessFailedEventArgs>| {
                    ...
                    log_webview_fail(kind_name);
                    eprintln!("[CrashShield] WebView2 进程异常 (kind={kind_name})");

                    // 浏览器内核主进程挂掉无法原地重载，其余渲染/卡死/GPU异常均可原生自愈恢复
                    if kind != COREWEBVIEW2_PROCESS_FAILED_KIND_BROWSER_PROCESS_EXITED {
                        let now_ms = std::time::SystemTime::now()
                            .duration_since(std::time::UNIX_EPOCH)
                            .map(|d| d.as_millis() as u64)
                            .unwrap_or(0);
                        let last_ms = LAST_HEAL_MS.load(Ordering::Relaxed);
                        if now_ms.saturating_sub(last_ms) >= MIN_HEAL_INTERVAL_MS {
                            LAST_HEAL_MS.store(now_ms, Ordering::Relaxed);
                            eprintln!("[CrashShield] 正在触发渲染管道原生自愈重载...");
                            if let Err(e) = core_clone.Reload() {
                                eprintln!("[CrashShield] core.Reload() 失败: {:?}, 降级尝试 eval 重载", e);
                                let _ = win_clone.eval("location.reload()");
                            }
                        } else {
                            eprintln!("[CrashShield] 重载触发过于频繁，跳过以避免雪崩");
                        }
                    }
            ...
            let res = core.add_ProcessFailed(&handler, &mut token);
            if res.is_ok() {
                println!("[CrashShield] WebView2 ProcessFailed 自愈保护已启用");
            } else {
                eprintln!("[CrashShield] 注册 ProcessFailed 失败: {:?}", res);
            }
        }
    });
}
...
#[cfg(not(target_os = "windows"))]
pub fn install_webview_shield(_window: &tauri::WebviewWindow) {}
